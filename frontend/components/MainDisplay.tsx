...
import { AppState, OperationMode, SystemConfig } from '../types';
          ...
          <div className="flex-1 relative bg-black">
            {state.isCameraOn ? (
              <video 
                ...
                playsInline 
                muted 
                className={`w-full h-full object-cover ${state.isAutoMatting ? 'brightness-125 contrast-110' : ''}`} 
              />
            ...
            {state.isAutoMatting && (
              <div className="absolute top-3 left-3 px-2 py-1 bg-blue-600/80 rounded-lg text-[8px] font-black text-white uppercase tracking-widest flex items-center space-x-2">
                <Scan size={10} /><span>Auto Matting Active</span>
              </div>
            )}
          </div>
        </div>
      ...
      <footer className="flex items-center justify-between h-14 md:h-16 px-4 md:px-6 overflow-x-auto scrollbar-hide select-none">
        ...
        <div className="flex items-center space-x-4 shrink-0">
          <div className="flex items-center space-x-3 px-3 py-2 bg-white/5 rounded-xl border border-white/10">
            ...
            <span className="text-[9px] font-black text-white/60 tracking-widest uppercase">安全环境</span>
          </div>
          <div className="flex items-center space-x-3 px-3 py-2 bg-white/5 rounded-xl border border-white/10">
            <Zap size={12} className="text-blue-400" />
            <span className="text-[9px] font-black text-white/60 tracking-widest uppercase">已联机</span>
          </div>
          <div className="text-[10px] font-mono font-bold text-green-400">12ms</div>
        </div>
      </footer>
